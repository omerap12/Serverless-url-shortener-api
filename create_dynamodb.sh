...
sed -i '' -e "s/8000:8000/$PORT:$PORT/" dynamo-db-configuration/docker-compose.yml
docker-compose -f dynamo-db-configuration/docker-compose.yml up &

echo "Create user-table"
aws dynamodb create-table --cli-input-json file://dynamo-db-configuration/create-table.json --endpoint-url http://localhost:"$PORT"
if [[ $? -ne 0 ]]; then
